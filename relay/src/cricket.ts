...
export function applyCricketBall(current: MatchState, payload: CricketBallEventPayload): CricketState {
  ...
  if (payload.isWide) {
    ...
    runsThisBall = 1 + payload.runs;
  } else if (payload.isNoBall) {
  ...
  inn.freeHit = payload.isNoBall ? true : false;
  ...
  if (legalDelivery) {
    ...
    const rotatingRuns = payload.isBye || payload.isLegBye ? payload.runs : (dismissalStands ? 0 : payload.runs);
    if (rotatingRuns % 2 === 1) {
      [inn.currentBatter1Index, inn.currentBatter2Index] = [inn.currentBatter2Index, inn.currentBatter1Index];
    }

    if (inn.ballsThisOver >= 6) {
      inn.oversComplete += 1;
      inn.ballsThisOver = 0;
      inn.thisOverBalls = [];
      if (bowler) {
