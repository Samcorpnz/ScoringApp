/**
 * Scoreboard Cloud Relay
 *
 * Socket events:
 *   Bridge  → Relay: "stateUpdate"      (MatchState)
 *   Relay   → All:   "matchStateChange"  (MatchState)
 *   Control → Relay: "manualUpdate"      (Partial<MatchState>)
 *   Control → Relay: "resetMatch"
 *   Relay   → Bridge: "manualUpdate"    (Partial<MatchState>)
 */
...
const CONTROL_SECRET = process.env.CONTROL_SECRET         ?? "changeme";
...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    // Always overwrite so the URL stays stable: /logos/home.png
...
app.post(
  ...
  (req, res) => {
    ...
    // Add cache-busting timestamp to URL so browsers reload after re-upload
...
app.delete("/api/logo/:team", logoUploadAuth, (req, res) => {
  const team = req.params.team as "home" | "visitor";
  // Remove any file matching home.* or visitor.*
...
io.on("connection", (socket) => {
  ...
  console.log(`[+] ${role} connected (${socket.id}) — ${io.engine.clientsCount} total`);
  ...
  if (isBridge) {
    ...
    socket.on("stateUpdate", (state: MatchState) => {
      if (state.sequenceId >= currentState.sequenceId) {
        // Preserve logo/colour settings that were set via control panel
        currentState = {
          ...state,
  ...
  if (isControl) {
    ...
    socket.on("resetMatch", () => {
      currentState = {
        ...
        // Preserve names, colours, logos across reset
